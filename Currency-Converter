def currency_converter(amount , from_currency, to_currency):
    ...
    if from_currency in conversion_rates and to in conversion_rates[from_currency]:
...
def main():
    ...
    from_currency =input(f"Ente the source currency({', '.join(currencies)})").upper()
    to_currency = input(f"Enter the target currency ({', '.join(currencies)}): ").upper()

    if from_currency not in currencies or to_currency not in currencies:
        print("Invalid Currency. please enter one of the suppored currencies. ")
        return
    converted_amount = currency_converter(amount, from_currency, to_currency)
    if converted_amount is not None:
        print(f"{amount} {from_currency} is equal to {converted_amount:.2f} {to_currency}")
    else:
        print("Conversion error. PLeses chceck your input.")
